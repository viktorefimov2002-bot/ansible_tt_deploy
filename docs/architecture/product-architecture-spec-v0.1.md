...
Документ описывает целевую архитектуру MVP и требования к её реализации. На момент утверждения baseline репозиторий содержит Ansible/CLI-автоматизацию. Наличие описания компонента не означает его реализации; фактическое состояние определяется кодом и конфигурацией репозитория.
...
Для целевой архитектуры MVP выбран Redis.
...
Планируемая реализация инфраструктурных адаптеров в MVP:
...
Целевое развёртывание Control Plane для MVP:
...
`device_limit` — параметр квоты устройств VPN-пользователя; значение по умолчанию — 3. Проверки используют значение параметра, а не жёстко заданную константу. Реализация должна допускать изменение или настройку `device_limit` в будущем в соответствии со спецификацией продукта.
...
Квота устройств не ограничивает суммарное число credentials или конфигураций. Для каждого доступного VPN-сервера у устройства могут быть отдельные credentials или конфигурация. При доступе к N серверам пользователь с 3 устройствами может иметь до 3 × N связей Device × Server.
