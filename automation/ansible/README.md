...
## Размещение после TTCP-002

Все команды ниже выполняются **из корня репозитория**. Реализация находится
в automation/ansible/; корневые ttctl, bootstrap.sh, add_vpn_clients.sh,
remove_vpn_clients.sh, scripts/*.sh и плейбуки сохранены как обёртки.
Они не меняют текущий рабочий каталог: явно переданные относительные пути
разрешаются как раньше.

deploy.yml, inventory.ini, vars.yml, vault.yml, генерируемые deploy.sh/uninstall.sh,
.venv/, client_configs/ и deployment_info_*.md остаются в корне.
Готовые deploy/uninstall helpers, созданные до переноса, продолжают использовать
корневые перенаправления.

Примеры конфигурации теперь находятся в automation/ansible/.
Входные файлы роли теперь лежат в automation/ansible/roles/trusttunnel_endpoint/files/.
При обновлении существующего checkout **до следующего запуска** перенесите
свои игнорируемые git файлы из прежнего roles/trusttunnel_endpoint/files/
в этот каталог, включая credentials с нестандартными именами.
Не перезаписывайте файлы назначения: сначала сравните их локально.
Автоматическая миграция секретов не выполняется. Прежние и новые пути стандартных
credentials/input-файлов остаются в .gitignore.

Пользовательские inventory, group_vars/host_vars и ansible.cfg, если они
существуют только локально, требуют отдельной проверки оператором; в исходном
репозитории нет ansible.cfg и host_vars. Отслеживаемый group_vars/all.yml
перенесён вместе с плейбуками. Прямой запуск из корня:

```bash
ansible-playbook -i inventory.ini automation/ansible/site.yml -e @vars.yml
# Старый путь также поддерживается:
ansible-playbook -i inventory.ini site.yml -e @vars.yml
```

bootstrap.sh остаётся интерактивным CLI/Ansible helper.
Будущий bootstrap-node.sh в этой задаче не реализован.

## Проверка структуры без подключения к серверу

```bash
bash automation/ansible/tests/layout_smoke.sh
```

Проверка создаёт временный checkout, подменяет только команду Ansible для
записи аргументов и проверяет реальные CLI/helper-скрипты. Для проверки
`ttctl init` нужны `python3` и PyYAML; при их отсутствии выводится `SKIP`.
Это не проверка Ansible или удалённого provisioning.

На Linux/WSL с установленным Ansible проверьте обе точки входа каждого плейбука:

```bash
for playbook in site uninstall status add_clients remove_clients; do
  ansible-playbook --syntax-check -i automation/ansible/inventory.example.ini \
...
Он спрашивает отдельно про каждый компонент и не удаляет `python3` автоматически, потому что Python часто нужен системе и другим инструментам. Подробнее: `automation/ansible/docs/DEPENDENCIES.md`.
...
```text
automation/ansible/roles/trusttunnel_endpoint/files/new_clients.toml
```
...
1. если локального `automation/ansible/roles/trusttunnel_endpoint/files/credentials.toml` нет, он сам забирает текущий `/opt/trusttunnel/credentials.toml` с сервера через Ansible;
...
```text
automation/ansible/roles/trusttunnel_endpoint/files/remove_clients.txt
```
...
rm -f automation/ansible/roles/trusttunnel_endpoint/files/credentials.toml
```

Если PR уже был влит и нужно откатить git-изменения, смотри:

```text
automation/ansible/docs/ROLLBACK.md
```
...
client_configs/
automation/ansible/roles/trusttunnel_endpoint/files/credentials.toml
automation/ansible/roles/trusttunnel_endpoint/files/new_clients.toml
automation/ansible/roles/trusttunnel_endpoint/files/remove_clients.txt
```
