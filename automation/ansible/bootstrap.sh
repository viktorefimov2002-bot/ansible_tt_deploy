...
SCRIPT_DIR=$(CDPATH= cd -- "${ANSIBLE_DIR}/../.." && pwd)
...
CREDENTIALS_FILES_DIR="${ANSIBLE_DIR}/roles/trusttunnel_endpoint/files"
VAULT_PASSWORD_FILE=""

require_tty() {
  if [ ! -r /dev/tty ]; then
    echo "This bootstrap helper must be run from an interactive terminal." >&2
    echo "For non-interactive usage, copy automation/ansible/vars.example.yml to vars.yml and run ansible-playbook manually." >&2
    exit 1
  fi
}
...
. "\${SCRIPT_DIR}/automation/ansible/helpers/ansible_playbook_helper.sh"
...
ansible-playbook -i "\${SCRIPT_DIR}/inventory.ini" "\${SCRIPT_DIR}/automation/ansible/${playbook_name}" "\${EXTRA_VARS[@]}" "\$@"
EOF
}
...
ansible_result=0
if [ "$vault_enabled" = "true" ]; then
  ansible-playbook -i "$INVENTORY_FILE" "$ANSIBLE_DIR/site.yml" -e "@$VARS_FILE" -e "@$VAULT_FILE" --vault-password-file "$VAULT_PASSWORD_FILE" || ansible_result=$?
else
  ansible-playbook -i "$INVENTORY_FILE" "$ANSIBLE_DIR/site.yml" -e "@$VARS_FILE" || ansible_result=$?
fi
